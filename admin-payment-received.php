<?php
// admin-payment-received.php - Handle payment received notification
...
// Update payment report to include received notification with timestamp
$update_sql = "
UPDATE payment 
SET payment_report = CONCAT(
...
if (mysqli_stmt_execute($update_stmt)) {
    ...
    VALUES ('customer', ?, 'Payment Received', ?, ?, NOW())";
    
    $notif_message = "Great news! We have received your payment of ₱" . number_format($payment['amount'], 2) . " for " . $payment['appliance_type'] . " service. Your payment is being processed. Thank you!";
    ...
    header('Location: ' . $return_url . '?success=' . urlencode('Payment received notification sent to customer!'));
    exit;
} else {
    header('Location: ' . $return_url . '?error=' . urlencode('Failed to send notification: ' . mysqli_error($conn)));
    exit;
}
